...
$('#add-member-form').submit(function (event) {
    event.preventDefault(); // Ensure form does not submit normally
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                alert(res.msg);
                $('#memberModal').modal('hide'); // Hide modal
        ...
        error: function (xhr, status, error) {
            ...
            $('#add-member-error').text("An error occurred. Please try again.");
            setTimeout(() => {
                $('#add-member-error').text('');
            }, 3000);
        }
...
$('.group-list').click(function(){
	...
	global_group_id = $(this).attr('date-id');
			...
			if(response.success){
				...
				<div class="current-user-chat" id='`+response.chat._id+`'>
					<h5>
						...
						<i class="fa fa-edit editGroupChat" aria-hidden="true" data-id='`+response.chat._id+ `'
						data-msg='`+message+ `' data-toggle="modal" data-target="#editGroupChatModal"></i>


					</h5>`;


				let date = new Date(response.chat.createdAt); // Ensure correct property name

				let cDate = String(date.getDate()).padStart(2, '0'); // Ensures two digits
				let cMonth = String(date.getMonth() + 1).padStart(2, '0'); // Ensures two digits
				let cYear = date.getFullYear();
				
				let getFullDate = `${cDate}-${cMonth}-${cYear}`; // Template literal for readability
...
socket.on('loadNewGroupChat',function(data){ 
	if(global_group_id == data.group_id){
		...
		let html = `
			...
			let date = new Date(data.createdAt); // Ensure correct property name
			...
			if(res.success){
				...
				for(let i=0;i<chats.length;i++){
					...
					if(chats[i]['sender_id']._id == sender_id){
						...
						<i class="fa fa-edit editGroupChat" aria-hidden="true" data-id='`+chats[i]['_id']+ `'
						data-msg='`+chats[i]['message']+ `' data-toggle="modal" data-target="#editGroupChatModal"></i>`;
						}	

					html +=`		   
			                </h5>`;

							let date = new Date(chats[i]['createdAt']); // Ensure correct property name
...
// listen delete chat is using socket
